...
* **prediction with trees** = iteratively split variables into groups (effectively constructing decision trees) --> produces nonlinear model
...
* as annotated above, the log-ratio is effectively an equation of a line for a set of predictor variables $x$ --> the first two terms are constants and the last term is in the form of $X \beta$
...
$$\begin{aligned}
\mbox{majority vote accuracy} & = probability(3~correct,~2~wrong) + probability(4~correct,~1~wrong) \\
&\qquad+ probability(5~correct) \\
